...
Made by Aditi and Molshree using Naive Bayes Classifier 
<p>Our project focused on predicting the level of depression in individuals based on their Twitter tweets, employing Machine Learning (ML) and Natural Language Processing (NLP) techniques. The key algorithm utilized was the Naive Bayes Classifier, which we implemented from scratch. The methodology involved data preprocessing, including the removal of stop words and extraneous characters. Additionally, each word underwent lemmatization to enhance the accuracy of the analysis.
